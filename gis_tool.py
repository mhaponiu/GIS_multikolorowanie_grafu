...
#     https: // docs.python.org / 2 / library / argparse.html  # other-utilities
    ...
    gen_parser.add_argument('-E', '--edges', help="~max liczba wylosowanych krawedzi dla wezla (default: %(default)s, type: %(type)s)"
                            , type=int, default=4, metavar='e')
    gen_parser.add_argument('-C', '--colors', help="max liczba wylosowanych kolorow dla wezla (default: %(default)s, type: %(type)s)"
    ...
    if 'generuj' in argumenty:
        ...
        if zparsowane.PNG:
            rysuj_graf_wejsciowy(g, zparsowane.Output_file+'.png', size=(int(zparsowane.PNG[0]), int(zparsowane.PNG[1])))
